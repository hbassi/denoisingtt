...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_sample = torch.tensor(self.input_data[0], dtype=torch.float32)  # Shape: [time_steps, 1, 48, 48]
        target_sample = torch.tensor(self.target_data[0], dtype=torch.float32)  # Shape: [time_steps, 1, 128, 128]
...
batch_size = 16
train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False)  # No shuffle as we pick random trajectory
