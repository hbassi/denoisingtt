...
def load_data():
    ...
    input_CG = np.load('/pscratch/sd/h/hbassi/GS_model_multi_traj_data_coarse_scale_dynamics_1500_tmax=5_sigma=7_numterms=20.npy').reshape((1447, 50, 1, 48, 48))[:1000, :5, :, :, :]
    target_FG = np.load('/pscratch/sd/h/hbassi/GS_model_multi_traj_data_fine_scale_dynamics_1500_tmax=5_sigma=7_numterms=20.npy').reshape((1447, 50, 1, 128, 128))[:1000, :5, :, :, :]
...
def train_model():
    ...
    train_ds, val_ds = random_split(dataset, [77, 1])
    ...
    for epoch in trange(num_epochs + 1):
        ...
        if epoch % 1000 == 0:
            ...
            }, f"/pscratch/sd/h/hbassi/vlasov_unet_checkpoint_epoch_{epoch}_without_compile.pth")
            
            # Save best model
            if avg_val_loss < best_val_loss:
                best_val_loss = avg_val_loss
                torch.save(model.state_dict(), "/pscratch/sd/h/hbassi/vlasov_unet_best_model_without_compile.pth")
